...
class FilmForm(forms.ModelForm):
    class Meta:
        ...
        fields = '__all__'
        widgets = {
            'film_director': forms.Select(attrs={'class': 'form-control'}),
    ...
    def clean_title(self):
        ...
        if Film.objects.filter(title=title).count() > 0:
            raise ValidationError('Film with this name already exists')
        return title
...
class UserCreateForm(forms.Form):
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username=username):
            raise ValidationError('User already exist with this username')
        return username
